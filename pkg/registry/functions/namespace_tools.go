	...
	return []server.ServerTool{
		...
		},
		{
			...
			Tool: mcp.NewTool("functions-list-access-keys",
				mcp.WithDescription("List access keys for a DigitalOcean Functions namespace. Returns metadata only (name, id, creation/expiry timestamps) — secret values are NOT returned and cannot be retrieved once a key has been created.\n\nUse this tool to:\n- Check whether a namespace already has access keys.\n- Find stale agent-created keys (names starting with `mcp-agent-`) for cleanup before creating a new one.\n\nNote: keys whose names start with `mcp-do-` are reserved for this MCP server's own internal use. They are managed automatically and must not be deleted by agents.\n\nHard limit: 200 access keys per account."),
				mcp.WithString("NamespaceID", mcp.Required(), mcp.Description("The UUID of the namespace")),
			),
		},
		{
			...
			Tool: mcp.NewTool("functions-create-access-key",
				mcp.WithDescription("Create an access key for a DigitalOcean Functions namespace. The returned secret appears only in this response and cannot be retrieved later — store it immediately.\n\nTypical use: creating a short-lived credential so `doctl serverless connect <namespace> --access-key dof_v1_<id>:<secret>` can connect without a full DigitalOcean API token.\n\nWhen creating a key for automated / agent use:\n- Name it with the prefix `mcp-agent-` followed by a timestamp (e.g. `mcp-agent-20260421-143000`) so it can be identified and cleaned up. Do NOT use the `mcp-do-` prefix — that is reserved for the MCP server's internal keys, and keys you create with that prefix will be auto-deleted on the next MCP call that touches the namespace.\n- Set ExpiresIn to \"24h\" unless the user explicitly requests a longer duration.\n- Before creating, call `functions-list-access-keys` and `functions-delete-access-key` to remove any existing keys whose names start with `mcp-agent-`, to avoid accumulating keys (hard limit: 200 per account).\n\nRequires the `function:admin` scope on the caller's API token."),
				mcp.WithString("NamespaceID", mcp.Required(), mcp.Description("The UUID of the namespace")),
